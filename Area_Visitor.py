...
MQTT_HOST = "127.0.0.1"
MQTT_PORT = 1883
MQTT_CLIENT_ID = "area_visitor_camera"
MQTT_SUBSCRIBE_TOPIC = "AIparking/area_visitor"
...
cinUrl = f"{cntUrl}/KETI_Block_Visitor"
...
def createCnt():
    ...
    payload = {
        "m2m:cnt": {
            "rn": "KETI_Block_Visitor",
            "lbl": ["KETI_Block_Visitor"]
        }
    }
    headers = {
        'Accept': 'application/json',
...
def on_message(client, userdata, msg):
    """
    Area07, Area08(228~231번)로부터 area_visitor 토픽 수신
    """
    ...
    try:
        ...
        # Area07, Area08에서 받은 area_visitor 토픽 데이터 저장
        with mqtt_data_lock:
...
def integrate_parking_data(own_parking_status):
    """
    Area_Visitor과 토픽 area_visitor 데이터를 통합
    - own_parking_status: Area_Visitor 카메라로 감지한 주차 상태 (212~220, 222~227)
    - integrated_parking_status: 토픽 area_visitor 데이터 (228~231)
    """
    global integrated_parking_status
    
    complete_status = {}
    
    # 방문객 주차장 데이터 추가 (212~220, 222~227)
    for spot_id, status in own_parking_status.items():
        spot_num = int(spot_id)
        if (212 <= spot_num <= 220) or (222 <= spot_num <= 227):
            complete_status[spot_id] = status
    
    # 토픽 area_visitor 데이터 추가 (228~231)
    with mqtt_data_lock:
...
def send_to_mobius(parking_status):
    ...
    if success:
        print(f"[Mobius] Area_Visitor complete data sent: {len(mobius_payload)} spots")
        # 전송 성공 시 마지막 전송 시간 업데이트
        last_mobius_send.update(parking_status)
    else:
        print(f"[Mobius] Failed to send Area_Visitor data")
...
def run_detection_loop(model, cap, parking_lines, original_parking_lines, rtsp_url):
    ...
    while True:
        ...
        cv2.imshow('Area_Visitor Detection', result_image)
...
def main():
    ...
    model = YOLO('./pt/Area_visitor.pt')
    model.to(device)

    rtsp_url = 'rtsp://admin:1234556@10.252.11.39/stream0'
    cap = cv2.VideoCapture(rtsp_url)

    # 전처리 매핑 파일 로딩(DIoU 판단용)
    mapping_file_path = './mapping/Area_visitor_mapping.txt'
    parking_lines = load_parking_lines(mapping_file_path)
    
    # 원본 시각화 매핑 파일 로딩
    original_mapping_file_path = './mapping/Area_visitor_mapping_original.txt'
