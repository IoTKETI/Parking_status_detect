...
def preprocess_image(image):
    ...
    # 2. 원근 변환
    pts = np.array([[235, 165], [1615, 235], [1824, 770], [145, 907]], dtype=np.float32)
    sm = pts.sum(axis=1)
...
def run_detection_loop(model, cap, parking_lines, original_parking_lines, rtsp_url):
    ...
    while True:
        ...
        cv2.imshow('Area03 Detection', result_image)
...
def main():
    ...
    model = YOLO('./pt/Area03.pt')
    model.to(device)

    # RTSP 연결
    rtsp_url = 'rtsp://admin:1234556@10.252.73.32/stream0'
    cap = cv2.VideoCapture(rtsp_url)

    # 전처리 매핑 파일 로딩 (DIoU 판단용)
    mapping_file_path = './mapping/Area03_mapping.txt'
    parking_lines = load_parking_lines(mapping_file_path)

    # 원본 시각화 매핑 파일 로딩
    original_mapping_file_path = './mapping/Area03_mapping_original.txt'
