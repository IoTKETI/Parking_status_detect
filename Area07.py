...
MQTT_HOST = "127.0.0.1"
MQTT_PORT = 1883
MQTT_CLIENT_ID = "area07_camera"
MQTT_PUBLISH_TOPIC1 = "AIparking/area06"
MQTT_PUBLISH_TOPIC2 = "AIparking/area08"
MQTT_PUBLISH_TOPIC3 = "AIparking/area_visitor"
...
def integrate_parking_data(own_parking_status):
    """
    7주차장 데이터만 추출(164~190 주차면)
    - own_parking_status: 카메라로 탐지한 주차 상태
    """
    # 7주차장 데이터만 추출 (164~190)
    complete_status = {}
    
    for spot_id, status in own_parking_status.items():
        if 164 <= int(spot_id) <= 190:
...
def send_to_mobius(parking_status):
    ...
    if success:
        ...
        print(f"[Mobius] Failed to send Area07 data")
    
    return success

def build_payloads(parking_status, diou_scores):
    """
    각 토픽별로 전송할 JSON payload를 생성하는 함수
    - Area06: 151~163번
    - Area08: 207~211번
    - Area_Visitor: 228~231번
    """
    ...
    for spot_id, status in parking_status.items():
        if 151 <= int(spot_id) <= 163:
            area06_status[spot_id] = status
            area06_diou[spot_id] = diou_scores.get(spot_id, 0.0)
    
    payload_area06 = {
        "ts": timestamp,
        "parking_status": area06_status,
        "diou_scores": area06_diou
    }
    ...
    area08_diou = {}
    for spot_id, status in parking_status.items():
        if 207 <= int(spot_id) <= 211:
            area08_status[spot_id] = status
            area08_diou[spot_id] = diou_scores.get(spot_id, 0.0)
    
    payload_area08 = {
        "ts": timestamp,
        "parking_status": area08_status,
        "diou_scores": area08_diou
    }
    
    # Area_Visitor용 데이터(228~231)
    area_visitor_status = {}
    area_visitor_diou = {}
    for spot_id, status in parking_status.items():
        if 228 <= int(spot_id) <= 231:
            area_visitor_status[spot_id] = status
            area_visitor_diou[spot_id] = diou_scores.get(spot_id, 0.0)
    
    payload_area_visitor = {
        "ts": timestamp,
        "parking_status": area_visitor_status,
        "diou_scores": area_visitor_diou
    }
    
    return payload_area06, payload_area08, payload_area_visitor

def publish_to_mqtt(payload_area06, payload_area08, payload_area_visitor):
    """
    Area07 데이터를 3개의 MQTT 토픽으로 발행
    - Area06: 151 ~ 163번 데이터를 AIparking/area06 토픽으로 전송
    - Area08: 207 ~ 211번 데이터를 AIparking/area08 토픽으로 전송
    - Area_Visitor: 228 ~ 231번 데이터를 AIparking/area_visitor 토픽으로 전송
    """
    global mqtt_client
    ...
    try:
        # Area06 토픽으로 전송(151~163)
        if payload_area06["parking_status"]:
            payload_json = json.dumps(payload_area06)
            mqtt_client.publish(MQTT_PUBLISH_TOPIC1, payload=payload_json)
        
        # Area08 토픽으로 전송(207~211)
        if payload_area08["parking_status"]:
            payload_json = json.dumps(payload_area08)
            mqtt_client.publish(MQTT_PUBLISH_TOPIC2, payload=payload_json)
        
        # Area_Visitor 토픽으로 전송(228~231)
        if payload_area_visitor["parking_status"]:
            payload_json = json.dumps(payload_area_visitor)
            mqtt_client.publish(MQTT_PUBLISH_TOPIC3, payload=payload_json)
...
def preprocess_image(image):
    ...
    # 원근 변환
    pts = np.array([[610, 1], [1510, 1], [1917, 860], [55, 900]], dtype=np.float32)
    sm = pts.sum(axis=1)
    ...
    final_transformed_image = cv2.warpPerspective(image, mtrx, (w, h))
...
def draw_status_on_image(image, parking_lines, parking_status, diou_scores):
    ...
    for points, spot_id in parking_lines:
        ...
        # cv2.putText(result_image, status, (tx + 5, ty + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.4, text_color, 2)
        # cv2.putText(result_image, f"DIoU: {diou:.2f}", (tx + 5, ty + 55), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 2)
...
def run_detection_loop(model, cap, parking_lines, original_parking_lines, rtsp_url):
    ...
    133번은 MQTT로 전송, 146~163은 MQTT 데이터와 통합 후 Mobius 전송 -> 시각화
    """
    ...
    while True:
        ...
        if not initial_frame_read or (frame_counter % FRAME_SKIP == 0):
            ...
            # 초기 다른 주차장 주차면 데이터를 MQTT로 전송
            payload_area06, payload_area08, payload_area_visitor = build_payloads(parking_status, diou_scores)
            publish_to_mqtt(payload_area06, payload_area08, payload_area_visitor)
        ...
        cv2.imshow('Area07 Detection', result_image)
...
def main():
    ...
    model = YOLO('./pt/Area07.pt')
    model.to(device)
    
    # RTSP 연결
    rtsp_url = 'rtsp://admin:1234556@10.252.11.36/stream0'
    cap = cv2.VideoCapture(rtsp_url)

    # 전처리 매핑 파일 로딩 (DIoU 판단용)
    mapping_file_path = './mapping/Area07_mapping.txt'
    parking_lines = load_parking_lines(mapping_file_path)
    
    # 원본 시각화 매핑 파일 로딩
    original_mapping_file_path = './mapping/Area07_mapping_original.txt'
