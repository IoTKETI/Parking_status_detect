...
MQTT_HOST = "127.0.0.1"
MQTT_PORT = 1883
MQTT_CLIENT_ID = "area08_camera"
MQTT_PUBLISH_TOPIC = "AIparking/area_visitor" 
MQTT_SUBSCRIBE_TOPIC = "AIparking/area08"
...
def integrate_parking_data(own_parking_status):
    ...
    - own_parking_status: 자신의 카메라로 감지한 주차 상태 (191~206, 230)
    - integrated_parking_status: MQTT에서 받은 다른 카메라 데이터 (207~211)
    """
    global integrated_parking_status
    
    complete_status = {}
    
    # 8주차장 데이터 추가 (191~206만, 230은 제외)
    for spot_id, status in own_parking_status.items():
        if spot_id != "230" and 191 <= int(spot_id) <= 206:
            complete_status[spot_id] = status
    
    # MQTT에서 받은 데이터 추가 (207~211)
    with mqtt_data_lock:
...
def send_to_mobius(parking_status):
    ...
    if success:
        ...
        print(f"[Mobius] Failed to send Area08 data")
...
def build_payload(parking_status, diou_scores):
    ...
    - 230 데이터만 MQTT로 전송 (Area_Visitor에서 사용)
    """
    ...
    if "230" in parking_status:
        spot_230_status["230"] = parking_status["230"]
        spot_230_diou["230"] = diou_scores.get("230", 0.0)

    return {
        "ts": datetime.now(timezone.utc).isoformat(timespec="milliseconds"),
        "parking_status": spot_230_status,
        "diou_scores": spot_230_diou
    }
...
def draw_status_on_image(image, parking_lines, parking_status, diou_scores):
    ...
    # Area08 주차구역만 표시 (230번 제외)
    area06_parking_lines = [(points, spot_id) for points, spot_id in parking_lines if spot_id != "230"]
...
def run_detection_loop(model, cap, parking_lines, original_parking_lines, rtsp_url):
    ...
    230번은 MQTT로 전송, 191~206은 MQTT 데이터(207~211)와 통합 후 Mobius 전송 -> 시각화
    """
    ...
    while True:
        ...
        if not initial_frame_read or (frame_counter % FRAME_SKIP == 0):
            ...
            # 230번 주차면 데이터를 MQTT로 전송 (Area_Visitor에서 사용)
            payload = build_payload(parking_status, diou_scores)
            publish_status(payload)
            
            # 191~206 데이터를 MQTT 데이터(207~211)와 통합
...
def main():
    ...
    model = YOLO('./pt/Area08.pt')
    model.to(device)

    # RTSP 연결
    rtsp_url = 'rtsp://admin:1234556@10.252.11.40/stream0'
    cap = cv2.VideoCapture(rtsp_url)

    # 전처리 매핑 파일 로딩 (DIoU 판단용)
    mapping_file_path = './mapping/Area08_mapping.txt'
    parking_lines = load_parking_lines(mapping_file_path)
    
    # 원본 시각화 매핑 파일 로딩
    original_mapping_file_path = './mapping/Area08_mapping_original.txt'
