...
cinUrl = f"{cntUrl}/KETI_Block_B"

# 이전 주차 상태 저장
previous_parking_status = {}
...
def preprocess_image(image):
    ...
    dist_coeffs = np.array([-0.23, 0.1, 0, 0], dtype=np.float32)
    h, w = image.shape[:2]
    new_camera_matrix, _ = cv2.getOptimalNewCameraMatrix(camera_matrix, dist_coeffs, (w, h), 1, (w, h))
    undistorted_image = cv2.undistort(image, camera_matrix, dist_coeffs, None, new_camera_matrix)

    # 2. 첫 번째 원근 변환
    pts = np.array([[1856, 5], [1860, 1070], [162, 104], [34, 922]], dtype=np.float32)
    sm = pts.sum(axis=1)
    ...
    scanned_image = cv2.warpPerspective(undistorted_image, mtrx, (w, h))

    # 3. 두 번째 원근 변환
    src_pts = np.array([[327, 125], [1727, 16], [1850, 1005], [171, 1005]], dtype=np.float32)
    dst_pts = np.array([[0, 0], [w - 1, 0], [w - 1, h - 1], [0, h - 1]], dtype=np.float32)
    final_mtrx = cv2.getPerspectiveTransform(src_pts, dst_pts)
    final_transformed_image = cv2.warpPerspective(scanned_image, final_mtrx, (w, h))
    ...
    for (px1, py1, px2, py2), spot_id in parking_lines:
        ...
        for box in boxes:
            ...
            if px1 < x2 and px2 > x1 and py1 < y2 and py2 > y1:
                ...
                # DIoU 값이 임계값(0.4) 이상이면 주차된 것으로 간주
                if diou >= 0.4:
                    is_occupied = True
                    break
...
def run_detection_loop(model, cap, parking_lines, original_parking_lines, rtsp_url):
    ...
    while True:
        ...
        if not ret or not is_frame_valid(frame):
            retry_count += 1
            print(f"Error: Failed to read frame from RTSP stream or bad frame received. Reconnecting... (Attempt #{retry_count})")
            cap.release()
        ...
        if not initial_frame_read or (frame_counter % FRAME_SKIP == 0):
            ...
            # Mobius 전송(상태가 변경된 경우에만)
            if parking_status != previous_parking_status:
                createCin(parking_status)
        ...
        cv2.imshow('Area02 Detection', result_image)
...
def main():
    ...
    model = YOLO('./pt/Area02.pt')
    model.to(device)
    
    # RTSP 연결
    rtsp_url = 'rtsp://admin:1234556@10.252.73.31/stream0'
    cap = cv2.VideoCapture(rtsp_url)
    
    # 전처리 매핑 파일 로딩 (DIoU 판단용)
    mapping_file_path = './mapping/Area02_mapping.txt'
    parking_lines = load_parking_lines(mapping_file_path)
    
    # 원본 시각화 매핑 파일 로딩
    original_mapping_file_path = './mapping/Area02_mapping_original.txt'
