...
MQTT_HOST = "127.0.0.1"
MQTT_PORT = 1883
MQTT_CLIENT_ID = "area05_2_camera"
...
def build_payload(parking_status, diou_scores):
    ...
    - Area05_2의 126~130, 134~138번 데이터를 MQTT로 전송(DIoU 포함)
    """
    ...
    # 126~130, 134~138 데이터를 추출
    filtered_status = {}
    filtered_diou = {}
    
    for spot_id in ["126", "127", "128", "129", "130", "134", "135", "136", "137", "138"]:
...
def publish_status(payload_dict):
    """
    Area05_2 데이터를 MQTT로 발행
    - 126~129, 134~137번 주차면 상태 전송
    """
    global mqtt_client
    ...
    try:
        ...
        info = mqtt_client.publish(MQTT_PUBLISH_TOPIC, payload=payload)
        return True
    except Exception as e:
...
def preprocess_image(image):
    ...
    # 원근 변환
    pts = np.array([[313, 182], [1517, 126], [1542, 959], [181, 880]], dtype=np.float32)
    sm = pts.sum(axis=1)
...
def run_detection_loop(model, cap, parking_lines, original_parking_lines, rtsp_url):
    ...
    while True:
        ...
        cv2.imshow('Area05_2 Detection', result_image)
...
def main():
    ...
    model = YOLO('./pt/Area05_2.pt')
    model.to(device)
    
    # RTSP 연결
    rtsp_url = 'rtsp://admin:1234556@10.252.11.35/stream0'
    cap = cv2.VideoCapture(rtsp_url)
    
    # 전처리 매핑 파일 로딩 (DIoU 판단용)
    mapping_file_path = './mapping/Area05_2_mapping.txt'
    parking_lines = load_parking_lines(mapping_file_path)
    
    # 원본 시각화 매핑 파일 로딩
    original_mapping_file_path = './mapping/Area05_2_mapping_original.txt'
