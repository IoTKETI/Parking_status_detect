...
MQTT_HOST = "127.0.0.1"
MQTT_PORT = 1883
MQTT_CLIENT_ID = "area04_1_camera"
MQTT_SUBSCRIBE_TOPIC = "AIparking/area04"
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # Area04_2에서 받은 area04 토픽 데이터 저장
        with mqtt_data_lock:
...
def integrate_parking_data(area04_1_parking_status, area04_1_diou_scores=None):
    """
    Area04_1과 Area04_2의 데이터를 통합
    - area04_1_parking_status: Area04_1 카메라로  탐지한 주차 상태(115~125)
    - area04_1_diou_scores: Area04_1 카메라 DIoU 점수
    - integrated_parking_status: Area04_2에서 받은 데이터(113~115)
    - integrated_diou_scores: Area04_2 DIoU 점수
    - 겹치는 주차면(115): DIoU 점수 비교하여 높은 값 선택
    """
    global integrated_parking_status, integrated_diou_scores
    
    if area04_1_diou_scores is None:
        area04_1_diou_scores = {}
    
    complete_status = {}
    
    # Area04_1 데이터 추가(116~125)
    for spot_id, status in area04_1_parking_status.items():
        if 116 <= int(spot_id) <= 125:
            complete_status[spot_id] = status
    
    with mqtt_data_lock:
        # Area04_2 데이터 추가(113~114)
        for spot_id, status in integrated_parking_status.items():
            if 113 <= int(spot_id) <= 114:
                complete_status[spot_id] = status
        
        # 115번: 두 카메라의 DIoU 점수를 비교해서 더 높은 값 선택
        if "115" in integrated_parking_status or "115" in area04_1_parking_status:
            area04_1_diou_115 = area04_1_diou_scores.get("115", 0.0)
            area04_2_diou_115 = integrated_diou_scores.get("115", 0.0)
            
            if area04_1_diou_115 >= area04_2_diou_115:
                if "115" in area04_1_parking_status:
                    complete_status["115"] = area04_1_parking_status["115"]
            else:
                if "115" in integrated_parking_status:
                    complete_status["115"] = integrated_parking_status["115"]
...
def send_to_mobius(parking_status):
    ...
    if success:
        ...
        print(f"[Mobius] Failed to send Area04 data")
...
def preprocess_image(image):
    ...
    # 1. 첫 번째 원근 변환
    pts = np.array([[60, 200], [1780, 50], [1810, 400], [160, 930]], dtype=np.float32)
    sm = pts.sum(axis=1)
    ...
    scanned_image = cv2.warpPerspective(image, mtrx, (w, h))

    # 2. sin파 변환(remap)
    # y' = y + A * sin(2π * wave_count * x / width)
    sin_image = cv2.remap(
        scanned_image, SIN_MAP_X, SIN_MAP_Y,
        interpolation=cv2.INTER_LINEAR,
        borderMode=cv2.BORDER_CONSTANT
    )

    # 3. 두 번째 원근 변환
    src_pts = np.array([[120, 100], [1800, 170], [1800, 930], [86, 900]], dtype=np.float32)
    dst_pts = np.array([[0, 0], [w - 1, 0], [w - 1, h - 1], [0, h - 1]], dtype=np.float32)
    final_mtrx = cv2.getPerspectiveTransform(src_pts, dst_pts)
    final_transformed_image = cv2.warpPerspective(sin_image, final_mtrx, (w, h))

    return final_transformed_image

def generate_sin_remap_map(width, height, amplitude, wave_count):
    """
    sin remap 맵 생성 함수
    """
    map_x = np.zeros((height, width), dtype=np.float32)
    map_y = np.zeros((height, width), dtype=np.float32)
    frequency = 2 * math.pi * wave_count / width

    for y in range(height):
        for x in range(width):
            map_x[y, x] = x
            map_y[y, x] = y + amplitude * math.sin(frequency * x)  # sin 반전 방향 (곡선 펼침)
...
def run_detection_loop(model, cap, parking_lines, original_parking_lines, rtsp_url):
    """
    주차 탐지 메인 루프 실행
    RTSP 스트림 수신 -> 전처리 -> YOLO 차량 탐지 -> DIoU 기반 주차 상태 판단 -> 시각화
    """
    ...
    while True:
        ...
        cv2.imshow('Area04_1 Detection', result_image)
...
def main():
    global previous_parking_status
    global SIN_MAP_X, SIN_MAP_Y
    
    # sin remap 맵을 미리 계산
    SIN_MAP_X, SIN_MAP_Y = generate_sin_remap_map(1920, 1080, SIN_AMPLITUDE, SIN_WAVE_COUNT)
    ...
    model = YOLO('./pt/Area04_1.pt')
    model.to(device)
    
    # RTSP 연결
    rtsp_url = 'rtsp://admin:1234556@10.252.11.37/stream0'
    cap = cv2.VideoCapture(rtsp_url)
    
    # 전처리 매핑 파일 로딩 (DIoU 판단용)
    mapping_file_path = './mapping/Area04_1_mapping.txt'
    parking_lines = load_parking_lines(mapping_file_path)
    
    # 원본 시각화 매핑 파일 로딩
    original_mapping_file_path = './mapping/Area04_1_mapping_original.txt'
